...
debug_link_flags := debug_shared_flags + link_flags + ' -static-libasan'
release_shared_flags := '-O2 -std=gnu11'
release_compile_flags := release_shared_flags
release_link_flags := release_shared_flags + link_flags
...
link type="debug" static="dynamic":
    #!/usr/bin/env bash
    ...
    if [[ {{ type }} == "debug" ]]; then
      ...
      for file in {{ out_dir }}/*-release.o; do
        if [[ {{ static }} == "true" || {{ static }} == "static" ]]; then
          if [[ $file -nt {{ bin_dir }}/httiny-static ]]; then
            WILL_LINK=true
          fi
        else
          if [[ $file -nt {{ bin_dir }}/httiny || $file -nt {{ bin_dir }}/httiny-static ]]; then
            WILL_LINK=true
          fi
        fi
      done
    ...
    if [[ {{ static }} == "dynamic" || {{ static }} == "false" ]]; then
      ...
      echo -e "Linking {{ green }}{{ type }}{{ reset }} dynamically complete"
    else
      echo -e "Linking {{ green }}{{ type }}{{ reset }} statically"
      echo -e "Files to link: {{ green }}"
      printf "%s\n" "${FILES[@]}" | sed 's/^/    /'
      echo -e "{{ reset }}"
      gcc {{ out_dir }}/*-release.o {{ release_static_link_flags }} -o {{ bin_dir }}/httiny-static -fuse-ld=mold
      echo -e "Linking {{ green }}{{ type }}{{ reset }} statically complete"
    fi
...
build type="debug" force="false" threads=num_cpus() static="dynamic":
    ...
    just link {{ type }} {{ static }}

build_static force="false" threads=num_cpus():
    just build release {{ force }} {{ threads }} static
...
generate_asm force="false" threads=num_cpus():
    #!/usr/bin/env bash
    shopt -s globstar
    ...
    for file in {{ src_dir }}/**/*.c; do
      if [[ $file -nt  {{ asm_dir }}/$(basename "${file%.c}").S ]]; then
        WILL_COMPILE=true
      fi

      if ! [[ -f {{ out_dir }}/$(basename "${file%.c}").S ]]; then
        WILL_COMPILE=true
      fi
    done
    ...
    echo -e "Using {{ red }}{{ threads }}{{ reset }} threads"
    echo -e "Target: {{ green }}asm{{ reset }}"
    find {{ src_dir }} -name "*.c" -print0 | xargs -0 -P{{ threads }} -n1 \
      sh -c 'if [[ "$1" -nt "{{ out_dir }}/$(basename "${1%.c}").S" || {{ force }} == "true" || {{ force }} == "force" ]]; then echo -e "Compiling {{ green }}$1{{ reset }}..."; gcc {{ include_flags }} {{ release_compile_flags }} -fverbose-asm -S "$1" -o "{{ asm_dir }}/$(basename "${1%.c}").S"; fi' sh

# Cleans the out directory
clean:
    [[ ! -d {{ out_dir }} ]] || rm -rf {{ out_dir }}
    [[ ! -d {{ bin_dir }} ]] || rm -rf {{ bin_dir }}
    [[ ! -d {{ asm_dir }} ]] || rm -rf {{ asm_dir }}
