...
void __httiny_path_conf_grow(httiny_arena_t *arena,
  ...
  httiny_assert(path_conf_ptr != NULL && "Invalid path conf");

  httiny_handler_list_t *handler_conf_handler_list_ptr =
      path_conf_ptr->handler_list;
  httiny_path_list_t *handler_conf_path_list_ptr = path_conf_ptr->path_list;

  httiny_assert(path_conf_ptr->shared_capacity < new_capacity &&
                "Can't shrink a path conf");

  httiny_path_conf_t *new_path_conf =
      arena_push(arena, sizeof(httiny_path_conf_t));
  httiny_handler_list_t *new_handler_conf_handler_list =
      arena_push(arena, sizeof(httiny_handler_list_t));
  new_handler_conf_handler_list->size = 0;
  new_handler_conf_handler_list->handlers =
      arena_push(arena, sizeof(httiny_handler_t) * new_capacity);

  httiny_path_list_t *new_handler_conf_path_list =
      arena_push(arena, sizeof(httiny_path_list_t));
  new_handler_conf_path_list->size = 0;
  new_handler_conf_path_list->paths =
      arena_push(arena, sizeof(httiny_path_t) * new_capacity);

  // No need to iterate for both, since they share size.
  for (u64 i = 0; i < handler_conf_handler_list_ptr->size; i++)
    if (handler_conf_handler_list_ptr->handlers[i] != NULL &&
        handler_conf_path_list_ptr->paths[i] != NULL) {
      new_handler_conf_handler_list->handlers[i] =
          handler_conf_handler_list_ptr->handlers[i];
      new_handler_conf_path_list->paths[i] =
          handler_conf_path_list_ptr->paths[i];
    }

  new_path_conf->handler_list = new_handler_conf_handler_list;
  new_path_conf->path_list = new_handler_conf_path_list;
  new_path_conf->handler_list->size = handler_conf_handler_list_ptr->size;
  new_path_conf->path_list->size = handler_conf_path_list_ptr->size;
  new_path_conf->shared_capacity = new_capacity;
  new_path_conf->thread_arena = get_thread_arena(MiB(16), MiB(8));
...
httiny_path_conf_t *path_conf_new(httiny_arena_t *thread_arena) {
  ...
  httiny_handler_list_t *handler_conf_handler_list =
      arena_push(thread_arena, sizeof(httiny_handler_list_t));
  httiny_path_list_t *handler_conf_path_list =
      arena_push(thread_arena, sizeof(httiny_path_list_t));

  handler_conf_handler_list->size = 0;
  handler_conf_path_list->size = 0;

  handler_conf_handler_list->handlers =
      arena_push(thread_arena, sizeof(httiny_handler_t) * initial_capacity);
  handler_conf_path_list->paths =
      arena_push(thread_arena, sizeof(httiny_path_t) * initial_capacity);

  path_conf->handler_list = handler_conf_handler_list;
  path_conf->path_list = handler_conf_path_list;
...
httiny_path_conf_t *handler_register(httiny_path_conf_t **path_conf,
  ...
  if (state_nullable)
    new_handler->state = state_nullable;
